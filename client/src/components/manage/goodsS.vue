<template >
  <div class="goodsL">
    <el-table :data="goodsList">
      ...
      <el-table-column prop="title" label="商品名称" width="200" align="center">
      </el-table-column>
      <el-table-column prop="price" label="价格" width="100" align="center">
        <template slot-scope="scope"> {{ scope.row.price }} 元</template>
      </el-table-column>
      <el-table-column prop="desc" label="详情" width="400" align="center">
      </el-table-column>
  ...
  methods: {
    goods() {
      let res = this.$axios.get("http://127.0.0.1:7001/goods").then((res) => {
        this.goodsList = res.data.filter((item) => item.type == "2");
      });
    },
    //删除商品
    handleDelete(id) {
